...
<div class="py-12 space-y-6">
        ...
        <div class="bg-gradient-to-r from-green-600/10 to-emerald-600/10 border border-green-500/20 rounded-2xl p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Highlight Clips</h1>
        ...
        <div class="bg-gradient-to-r from-yellow-600/10 to-orange-600/10 border border-yellow-500/30 rounded-2xl p-6">
            ...
            <a href="{{ route('clips.show', $clipOfTheWeek) }}" class="block hover:opacity-80 transition">
                <div class="bg-gray-900/50 rounded-xl p-4">
                    <h3 class="text-xl font-bold text-white mb-2">{{ $clipOfTheWeek->title }}</h3>
                    <div class="flex items-center gap-4 text-sm text-gray-400">
                        <span>by {{ $clipOfTheWeek->user->name }}</span>
                        <span class="{{ $clipOfTheWeek->platform_color }}">{{ $clipOfTheWeek->platform_name }}</span>
                        <span>{{ $clipOfTheWeek->votes }} votes</span>
                    </div>
                </div>
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <a href="{{ route('clips.show', $clip) }}" class="bg-gray-800/50 border border-gray-700 hover:border-green-500/50 rounded-xl overflow-hidden transition group">
                <div class="aspect-video bg-gray-900 flex items-center justify-center relative">
                    ...
                    @if($clip->is_featured)
                    <span class="absolute top-2 right-2 px-2 py-1 bg-yellow-600 text-white text-xs font-bold rounded">FEATURED</span>
                    @endif
                </div>
                <div class="p-4">
