...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.achievements.create') }}" class="px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-xl transition flex items-center gap-2">
    ...
    <div class="glass-card rounded-xl p-4">
        <form action="{{ route('admin.achievements.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search achievements..." class="flex-1 min-w-[200px] px-3 py-2 bg-white/3 border border-white/10 rounded-lg text-white text-sm placeholder-gray-400 focus:outline-none focus:border-green-500">
            <select name="category" class="px-3 py-2 bg-white/3 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-green-500">
            ...
            @if(request()->hasAny(['search', 'category']))
                <a href="{{ route('admin.achievements.index') }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg text-sm transition">Clear</a>
            @endif
        </form>
    </div>
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        <span class="px-1.5 py-0.5 bg-white/5 text-gray-300 rounded text-[10px]">{{ ucfirst($achievement->category) }}</span>
