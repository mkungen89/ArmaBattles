...
<form action="{{ route('admin.tournaments.store') }}" method="POST" class="space-y-6">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full bg-white/5 border-white/10 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">{{ old('description') }}</textarea>
                </div>

                <div>
                    <label for="rules" class="block text-sm font-medium text-gray-400 mb-2">Rules</label>
                    <textarea name="rules" id="rules" rows="4"
                        class="w-full bg-white/5 border-white/10 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">{{ old('rules') }}</textarea>
                </div>
        ...
        <!-- Format & Settings -->
        <div class="glass-card rounded-xl p-6">
            <h2 class="text-sm font-semibold text-white uppercase tracking-wider mb-3">Format & settings</h2>
    ...
    <div class="flex gap-4">
        ...
        <a href="{{ route('admin.tournaments.index') }}" class="px-6 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl transition">
            Cancel
        </a>
    </div>
</form>
