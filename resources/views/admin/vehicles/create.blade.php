...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-white">Add Vehicle</h1>
    </div>
    ...
    <form action="{{ route('admin.vehicles.store') }}" method="POST" enctype="multipart/form-data" class="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="name" class="block text-sm font-medium text-gray-400 mb-2">Vehicle Name (from game) *</label>
                <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
            ...
            <div>
                ...
                <input type="text" name="display_name" id="display_name" value="{{ old('display_name') }}" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
            ...
            <div>
                ...
                <select name="vehicle_type" id="vehicle_type" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
                    ...
                    <option value="APC" {{ old('vehicle_type') === 'APC' ? 'selected' : '' }}>APC</option>
                    <option value="IFV" {{ old('vehicle_type') === 'IFV' ? 'selected' : '' }}>IFV</option>
                    <option value="Tank" {{ old('vehicle_type') === 'Tank' ? 'selected' : '' }}>Tank</option>
                    <option value="Helicopter" {{ old('vehicle_type') === 'Helicopter' ? 'selected' : '' }}>Helicopter</option>
            ...
            <div class="md:col-span-2">
                <label for="image" class="block text-sm font-medium text-gray-400 mb-2">Vehicle Image</label>
                <div class="flex items-center gap-4">
                    <div class="flex-1">
        ...
        <div class="mt-6 flex gap-3">
            <button type="submit" class="px-6 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg transition">Create Vehicle</button>
