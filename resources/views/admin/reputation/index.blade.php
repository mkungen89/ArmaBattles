...
<div class="space-y-6">
    ...
    <div class="glass-card rounded-xl p-4">
        <form action="{{ route('admin.reputation.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by player name..." class="flex-1 min-w-[200px] px-3 py-2 bg-white/3 border border-white/10 rounded-lg text-white text-sm placeholder-gray-400 focus:outline-none focus:border-green-500">
            <select name="tier" class="px-3 py-2 bg-white/3 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-green-500">
            ...
            @if(request()->hasAny(['search', 'tier']))
                <a href="{{ route('admin.reputation.index') }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-lg text-sm transition">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="glass-card rounded-xl overflow-hidden">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/3 {{ $rep->isFlagged() ? 'bg-red-500/5' : '' }}">
