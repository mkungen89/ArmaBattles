    ...
    <div class="glass-card rounded-xl p-4">
        <form action="{{ route('admin.game-stats.healing') }}" method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search healer or patient..." class="w-full bg-white/5 border-white/10 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
            </div>
            ...
            <a href="{{ route('admin.game-stats.healing') }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl transition">
                Clear
            </a>
            @endif
        </form>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/5">
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="px-2 py-1 bg-white/5 text-gray-300 text-xs rounded">{{ $event->item ?? '-' }}</span>
                    </td>
