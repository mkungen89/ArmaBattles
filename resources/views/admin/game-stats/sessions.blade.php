...
@section('title', 'Connection Events - Game Statistics')
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-white">Connection Events</h1>
        </div>
        <span class="text-gray-400">{{ $sessions->total() }} events</span>
    </div>

    {{-- Filters --}}
    <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
        <form action="{{ route('admin.game-stats.sessions') }}" method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by player name..." class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
            </div>
            <select name="event_type" class="bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2 focus:ring-green-500 focus:border-green-500">
                ...
                <option value="CONNECT" {{ request('event_type') === 'CONNECT' ? 'selected' : '' }}>Connect</option>
                <option value="DISCONNECT" {{ request('event_type') === 'DISCONNECT' ? 'selected' : '' }}>Disconnect</option>
            </select>
            ...
            <a href="{{ route('admin.game-stats.sessions') }}" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition">
                Clear
            </a>
            @endif
        </form>
        ...
        <table class="w-full">
            <thead class="bg-gray-700/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider">Platform</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Faction</th>
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr class="hover:bg-gray-700/30">
                    ...
                    <td class="px-4 py-3 text-center">
                        ...
                        <span class="inline-flex items-center gap-1 px-2 py-1 bg-green-500/20 text-green-400 text-xs rounded">
                        ...
                        @else
                        <span class="inline-flex items-center gap-1 px-2 py-1 bg-red-500/20 text-red-400 text-xs rounded">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <td class="px-4 py-3">
                        <span class="text-white font-medium">{{ $session->player_name }}</span>
                        @if($session->player_uuid)
                        <div class="text-xs text-gray-500">{{ Str::limit($session->player_uuid, 20) }}</div>
                        @endif
                    </td>
                    <td class="px-4 py-3 text-center">
                        @if($session->player_platform)
                        <span class="inline-flex items-center px-2 py-1 bg-blue-500/20 text-blue-400 text-xs rounded">
                            {{ ucfirst($session->player_platform) }}
                        </span>
                        @else
                        <span class="text-gray-500">-</span>
                        @endif
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-4 py-8 text-center text-gray-500">No connection events found</td>
                </tr>
                @endforelse
            </tbody>
        </table>
