...
<div class="space-y-6">
        ...
        <form method="GET" action="{{ route('admin.reports.index') }}" class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex items-center gap-1">
                ...
                <a href="{{ route('admin.reports.index', array_merge(request()->except('status', 'page'), $val !== 'all' ? ['status' => $val] : [])) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-medium transition {{ (request('status', 'all') === $val) ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-white/3 text-gray-400 hover:text-white border border-transparent' }}">
                    {{ $label }}
            ...
            <div class="flex-1 min-w-[200px]">
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search player name or reason..."
                           class="w-full pl-9 pr-3 py-2 bg-gray-900/50 border border-white/5 rounded-lg text-sm text-white placeholder-gray-600 focus:outline-none focus:border-green-500/50">
            ...
            @endif
            <button type="submit" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm transition">Search</button>
            @if(request('search'))
    ...
    <div class="glass-card rounded-xl overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                    <tr class="border-b border-white/5 bg-gray-900/30">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-white/5 hover:bg-white/3 transition {{ $report->status === 'pending' ? 'bg-yellow-500/5' : '' }}">
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.reports.show', $report) }}"
                               class="px-3 py-1.5 text-xs bg-white/5 hover:bg-white/10 text-white rounded-xl transition">
                                View
                            </a>
                        </td>
                    </tr>
