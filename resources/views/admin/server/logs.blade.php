...
<div x-data="logViewer()" x-init="init()" class="space-y-6">
    ...
    <div class="flex items-center gap-1 glass-card rounded-xl p-1.5">
        ...
        <a href="{{ route('admin.server.logs', $logType) }}"
           class="flex-1 text-center px-4 py-2 rounded-lg text-sm font-medium transition {{ $type === $logType ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'text-gray-400 hover:bg-white/5 hover:text-white border border-transparent' }}">
            {{ $label }}
        </a>
        @endforeach
    </div>
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-1">
                ...
                <a href="{{ route('admin.server.logs', $type) }}?lines={{ $lineCount }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-medium transition {{ request('lines', 100) == $lineCount ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'glass-card text-gray-400 hover:text-white' }}">
                    {{ $lineCount }}
                </a>
                @endforeach
            </div>
        ...
        <div class="flex items-center gap-2">
            <button @click="scrollToBottom()" class="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-xl text-xs transition flex items-center gap-1">
            ...
            <button @click="fetchLogs()" :disabled="loading" class="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-xl text-xs transition flex items-center gap-1.5 disabled:opacity-50">
    ...
    <div class="glass-card rounded-xl overflow-hidden">
        {{-- File info bar --}}
        <div class="px-4 py-2 border-b border-white/5 flex items-center justify-between bg-gray-900/30">
        ...
        <div x-ref="logContainer" class="p-4 max-h-[65vh] overflow-y-auto overflow-x-auto">
            ...
            <div x-show="!error" class="font-mono text-xs leading-relaxed space-y-0">
                <template x-for="(line, i) in logContent" :key="i">
                    <div class="text-gray-400 hover:bg-white/3 px-2 py-0.5 rounded whitespace-pre-wrap break-all flex">
