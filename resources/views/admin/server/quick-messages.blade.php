...
<div class="space-y-6">
        ...
        @if(!empty($templates) && count($templates) > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($templates as $template)
                    <div class="bg-gray-900/50 border border-white/5 rounded-lg p-4 flex flex-col justify-between">
                        <div class="mb-3">
    ...
    <div class="glass-card rounded-xl p-6" x-data="{ templates: @js($templates ?? []) }">
        <h2 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form action="{{ route('admin.server.quick-messages.save') }}" method="POST">
            ...
            <div class="space-y-3 mb-4">
                <template x-for="(template, index) in templates" :key="index">
                    <div class="flex items-center gap-3 bg-gray-900/50 border border-white/5 rounded-lg p-3">
                        <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text"
                                       ...
                                       class="w-full bg-white/5 border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-gray-500 focus:border-green-500 focus:ring-1 focus:ring-green-500 focus:outline-none">
                            </div>
                            <div>
            ...
            <div class="flex items-center justify-between">
                <button type="button"
                        ...
                        class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm font-medium transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
