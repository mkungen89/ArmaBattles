...
<div class="space-y-6">
    ...
    <div class="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
        <form action="{{ route('admin.clips.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by title..." class="flex-1 min-w-[200px] px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-white text-sm placeholder-gray-400 focus:outline-none focus:border-green-500">
            ...
            <select name="featured" class="px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:border-green-500">
            ...
            @if(request()->hasAny(['search', 'platform', 'featured']))
                <a href="{{ route('admin.clips.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-lg text-sm transition">Clear</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr class="hover:bg-gray-700/30">
                    ...
                    <td class="px-4 py-3">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $clip->platform_color }} bg-gray-700">
                            {{ $clip->platform_name }}
                        </span>
                    </td>
