    ...
    <form action="{{ route('players.search') }}" method="GET" class="flex gap-3">
        <div class="flex-1 relative">
            <input type="text" name="q" value="{{ $query }}" placeholder="Search by player name or UUID..."
                   class="w-full px-4 py-3 bg-white/5 border-white/10 border rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500">
    ...
    <div class="glass-card rounded-xl overflow-hidden">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="{{ $loop->odd ? 'bg-white/3' : 'bg-transparent' }} hover:bg-white/5 transition-colors">
                    <td class="px-4 py-3">
