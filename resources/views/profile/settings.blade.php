...
<div class="space-y-5">
    ...
    <div class="glass-card p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                <h3 class="text-sm font-semibold text-white uppercase tracking-wider mb-1">Linked Accounts</h3>
                <p class="text-gray-500 text-xs mb-4">
                    Connect your Steam and Google accounts to have multiple login options. You can sign in with any linked account.
                </p>
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 bg-white/3 rounded-xl border border-white/5">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-lg bg-[#171a21] flex items-center justify-center">
                            ...
                            <div>
                                <p class="text-sm font-medium text-white">Steam</p>
                                @if($user->steam_id)
                                    <p class="text-xs text-gray-400">Connected • {{ $user->name }}</p>
                                @else
                        ...
                        <div>
                            @if($user->steam_id)
                                @if($user->google_id)
                                    ...
                                    <form action="{{ route('profile.unlink-steam') }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                ...
                                @endif
                            @else
                                <a href="{{ route('auth.steam.link') }}"
                                   class="px-4 py-2 bg-[#171a21] hover:bg-[#212830] text-white text-xs rounded-lg transition font-medium inline-block">
                                    Link Steam
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center justify-between p-4 bg-white/3 rounded-xl border border-white/5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                @if($user->google_id)
                                    <p class="text-xs text-gray-400">Connected • {{ $user->google_email }}</p>
                                @else
                                    <p class="text-xs text-gray-500">Not connected</p>
                                @endif
                            </div>
                        ...
                        <div>
                            @if($user->google_id)
                                @if($user->steam_id)
                                    {{-- Can unlink if has another login method --}}
                                    <form action="{{ route('profile.unlink-google') }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="px-4 py-2 bg-red-600/80 hover:bg-red-500 text-white text-xs rounded-lg transition font-medium"
                            ...
                            @else
                                <a href="{{ route('auth.google.link') }}"
                                   class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded-lg transition font-medium inline-block">
                                    Link Google
                                </a>
                            @endif
                        </div>
